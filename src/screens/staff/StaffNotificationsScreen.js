...
export default function StaffNotificationsScreen({ navigation }) {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.ivory }} edges={['top']}>
      <ScreenHeader title="Notifications" onBack={() => navigation.goBack()} right={
        sorted.length > 0 ? (
          <TouchableOpacity onPress={markAllStaffNotificationsRead}><Text style={{ fontSize: 11, color: colors.turquoiseDark, fontWeight: '700' }}>Mark all</Text></TouchableOpacity>
        ) : null
      } />
      <FlatList
        data={sorted}
