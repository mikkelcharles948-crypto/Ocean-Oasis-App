  ...
  if (!destination) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: colors.ivory }}>
        <ErrorState title="Destination not found" onRetry={() => navigation.goBack()} />
      </SafeAreaView>
    );
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.ivory }}>
      <ScrollView showsVerticalScrollIndicator={false}>
        <View>
          <ImagePlaceholder kind={destination.image} style={{ height: 260, borderRadius: 0 }} iconSize={56} />
        ...
        <View style={styles.content}>
          <Badge label={destination.category} tone="info" />
          <Text style={styles.title}>{destination.title}</Text>
          <Text style={styles.description}>{destination.description}</Text>

          <View style={styles.statsRow}>
            <Stat icon="navigate-outline" label="Distance" value={destination.distance} />
            <Stat icon="time-outline" label="Duration" value={destination.duration} />
            <Stat icon="trending-up-outline" label="Difficulty" value={destination.difficulty} />
          </View>

          <View style={styles.noticeBox}>
            <Ionicons name="information-circle-outline" size={16} color={colors.turquoiseDark} />
            <Text style={styles.noticeText}>
              Exact pricing and availability vary by season. Ask our concierge for current details.
            </Text>
          </View>

          <Button label="Ask Concierge to Arrange" onPress={() => navigation.navigate('Concierge')} style={{ marginTop: spacing.lg }} />
          <Button
            label="View on Map"
            variant="outline"
